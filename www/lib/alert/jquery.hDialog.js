...
;(function($, window, document, undefined) {	
	...
	methods = {
        ...
        open: function (o,T) {
			...
			//关闭按钮
			if(o.closeHide != false) closeBtnTpl = '<a id="HCloseBtn" title="关闭" style="width:124px;display:inline-block;cursor:pointer;'+';font-size:15px;text-align:center;position:absolute;bottom:0px;left:0;"><span style="background-color:#f6f6f6; color:#1C6EDC;width:228px;height:32px;display:inline-block;border-radius:5px;line-height:32px;">取消</span></a>';
			...
			//遮罩背景层
			if(o.遮罩 != false) overlayTpl = "<div id='HOverlay' style='width:"+$D.width()+"px;height:"+$D.height()+"px;background-color:"+modalBg+";position:fixed;top:0;left:0;z-index:9999;'></div>";
			...
			if(o.types == 2){ iframeTpl = '<iframe width="' + o.width + '" height="' + o.height + '" frameborder="0" scrolling="auto" src="' + o.iframeSrc + '"></iframe>'; $B.append('<div id="'+o.iframeId+'"></div>'); $obj = $('#'+o.iframeId); }
        		...
        		$B.stop().append(overlayTpl).find($obj).css({'background-color':'#f6f6f6','position':'fixed','top':t,'left':l,'z-index':999999,'margin':m,'width':o.width,'height':o.height}).removeAttr('class').addClass('animated '+c).prepend(headTpl+closeBtnTpl+iframeTpl).show();
	...
	*/
	$.extend({  
        ...
        goTop: function (b, r) {
	        	...
	        	$B.stop().append('<a id="HGoTop" href="javascript:;" style="width:32px;height:32px;line-height:32px;border-radius:50%;display:inline-block;text-align:center;background:#333;color:#fff;position:fixed;bottom:'+b+';right:'+r+';z-index:100000;">Top</a>').find('#HGoTop').hide();
	        	$T = $('#HGoTop');
